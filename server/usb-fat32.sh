...
for TARGET in "${@:1}"; do
	...
	if [ "sd${TARGET}" == " $(echo ${PARTIMAG} | sed 's/[0-9]*//g')" ]; then
		echo "${RED}ERROR: WHY ARE YOU TRYING TO IMAGE THE DEVICE THAT HOLDS THE IMAGES!?!"
		echo "PLEASE TAKE SOME TIME TO REREAD THE DOCUMENTATION ***CAREFULLY***!${NC}"
		exit 1
	fi
